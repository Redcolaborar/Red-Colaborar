...
/**
  ...
  * https://www.npmjs.com/package/gulp-uglify
  */
gulp.task( 'bpjs', [ 'concat' ], () =>
	gulp.src( [ 'buddypress/js/*.js', '!buddypress/js/*.min.js' ] )
		.pipe( plumber( {'errorHandler': handleErrors} ) )
		.pipe( rename( {'suffix': '.min'} ) )
		.pipe( babel( {
			'presets': [
 ...
 */
gulp.task( 'clean:pot', () =>
	del( [ 'languages/redcolaborar.pot' ] )
);
...
gulp.task( 'wp-pot', [ 'clean:pot' ], () =>
	gulp.src( paths.php )
		...
		.pipe( gulp.dest( 'languages/redcolaborar.pot' ) )
);
...
gulp.task( 'watch', function() {
	...
	browserSync( {
		...
		'proxy': 'http://redcolaborar.dev',         // Use http://Red Colaborar.dev:3000 to use BrowserSync.
		'watchOptions': {
	...
	gulp.watch( [ 'buddypress/js/*.js', '!buddypress/js/*.min.js' ], [ 'bpjs' ] );
} );
