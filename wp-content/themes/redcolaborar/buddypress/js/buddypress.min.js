...
$whats_new.focus(function(){jq('#whats-new-options').slideDown();jq(this).animate({height:'3.8em'});jq('#aw-whats-new-submit').prop('disabled',!1);jq(this).parent().addClass('active');jq('#whats-new-content').addClass('active');var $whats_new_form=jq('form#whats-new-form'),$activity_all=jq('#activity-all');if($whats_new_form.hasClass('submitted')){$whats_new_form.removeClass('submitted')}
...
var wdsFilters=window.location.search.substr(1);if(wdsFilters){load_more_args.wdsFilters=wdsFilters}
bp_ajax_request=jq.post(ajaxurl,load_more_args,function(response)
{jq('#buddypress li.load-more').removeClass('loading');jq.cookie('bp-activity-oldestpage',oldest_page,{path:'/',secure:('https:'===window.location.protocol)});jq('#buddypress ul.activity-list').append(response.contents);target.parent().hide()},'json');return!1}
if(target.parent().hasClass('load-newest')){event.preventDefault();target.parent().hide();activity_html=jq.parseHTML(newest_activities);jq.each(activity_html,function(i,el){if('LI'===el.nodeName&&jq(el).hasClass('just-posted')){if(jq('#'+jq(el).attr('id')).length){jq('#'+jq(el).attr('id')).remove()}}});jq('#buddypress ul.activity-list').prepend(newest_activities);newest_activities=''}});jq('div.activity').on('click','.activity-read-more a',function(event){var target=jq(event.target),link_id=target.parent().attr('id').split('-'),a_id=link_id[3],type=link_id[0],inner_class,a_inner;inner_class=type==='acomment'?'acomment-content':'activity-inner';if(inner_class==='activity-inner'){a_inner=jq('#'+type+'-'+a_id+' .'+inner_class+':first .rc-activity-col-content')}else{a_inner=jq('#'+type+'-'+a_id+' .'+inner_class+':first')}
jq(target).addClass('loading');jq.post(ajaxurl,{action:'get_single_activity_content','activity_id':a_id},function(response){jq(a_inner).slideUp(300).html(response).slideDown(300)});return!1});jq('form.ac-form').hide();if(jq('.activity-comments').length){bp_legacy_theme_hide_comments()}
...
jq(button).removeClass('loading')});return!1});jq('body.messages #item-body div.messages').on('change','#message-type-select',function(){var selection=this.value,checkboxes=jq('td input[type="checkbox"]'),checked_value='checked';checkboxes.each(function(i){checkboxes[i].checked=''});switch(selection){case 'unread':checkboxes=jq('tr.unread td input[type="checkbox"]');break;case 'read':checkboxes=jq('tr.read td input[type="checkbox"]');break;case '':checked_value='';break}
...
comments_divs.each(function(){if(jq(this).find('li').length<3){return}
