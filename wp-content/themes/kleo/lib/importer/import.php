...
class kleoImport {
	...
	function add_initial_demo_sets() {
		...
		$pages_data['home-medical'] = array(
			...
			'attach' => 'yes',
			'plugins' => array( 'revslider', 'contact-form-7' ),
		...
		$pages_data['home-get-connected']          = array(
			...
			'attach'  => 'yes',
			'plugins' => array( 'buddypress', 'bp-profile-search' ),
		...
		$pages_data['home-agency']                 = array(
			...
			//'widgets' => 'yes',
			'attach'    => 'yes',
			'plugins'   => array( 'revslider', 'mailchimp-for-wp', 'contact-form-7' ),
	...
	public function import_assets() {
		if ( isset( $_GET['page'] ) && ( 'kleo_import' == $_GET['page'] || 'kleo-panel' == $_GET['page'] ) ) {
			...
			wp_enqueue_script( 'kleo-import-js', KLEO_LIB_URI . '/importer/assets/import.js', array( 'jquery', 'jquery-ui-tooltip' ), KLEO_THEME_VERSION, true );
		}
	...
	function do_ajax() {
		...
		if ( is_wp_error( $response ) ) {
			wp_send_json_error( array(
				'message' => $this->set_error_message( $response->get_error_message() ),
	...
	public function process_import( $options ) {
		...
		if ( isset( $data['import_widgets'] ) && isset( $set_data['widgets'] ) ) {
			...
			//widgets
			$file_path = KLEO_LIB_DIR . '/importer/demo/' . $widget_path . '.json';
		...
		if ( isset( $set_data['plugins'] ) && ! empty( $set_data['plugins'] ) ) {
			foreach ( $set_data['plugins'] as $plugin ) {
				...
				if ( ! SQ_Addons_Manager()->is_plugin_installed( $plugin ) ) {
					...
					$this->messages[] = sprintf( esc_html__( 'Installed plugin %s', 'kleo_framework' ), $plugin );
				}
				if ( ! SQ_Addons_Manager()->is_plugin_active( $plugin ) ) {
					...
					$this->messages[] = sprintf( esc_html__( 'Activated plugin %s', 'kleo_framework' ), $plugin );
				}
			}
		}
	}
	...
	function import_options( $file = '' ) {
		...
		$file_path 	= KLEO_LIB_DIR . '/importer/demo/options/' . $file . '.json';
		$file_data 	= sq_fs_get_contents( $file_path );
		...
		<div class="demos-wrapper">
			...
			<?php foreach ( $demo_sets as $k => $v ) : ?>
				...
				<div class="import-demo">
					<div class="demo-wrapper" <?php if ( isset( $v['slug'] ) ) echo 'data-slug="' . $v['slug'] . '"'; ?>>
						...
						<div class="img-wrapper">
							...
							<a href="<?php echo $v['link']; ?>" target="_blank">
								<span class="preview-btn">
									<span class="dashicons dashicons-visibility"></span> <?php esc_html_e( 'PREVIEW' , 'kleo_framework' ); ?>
								</span>
							</a>
						</div>
						<div class="demo-options">
							<div class="to-left">
								...
								<div class="demo-checkboxes">
									<?php if ( isset( $v['page'] ) ) : ?>
										<label><input type="checkbox" name="import_page[]" checked value="<?php echo $k; ?>"
										              class="check-page"> <?php esc_html_e( 'Import Page' , 'kleo_framework' ); ?></label>
										<br>
									<?php endif; ?>
									...
									<?php if ( isset( $v['extra'] ) && ! empty( $v['extra'] ) ) : ?>
										<?php foreach ( $v['extra'] as $extra ) : ?>
											...
											?>
											<label>
												<input type="checkbox" name="import_<?php echo $extra['id'];?>[]" value="<?php echo $k; ?>"
											              class="check-page"<?php echo $checked; ?>> <?php echo $extra['name']; ?></label>
											<br>
										<?php endforeach; ?>
									<?php endif; ?>
									...
									<?php if ( isset( $v['widgets'] ) ) : ?>
										<label><input type="checkbox" name="import_widgets[]" checked
										              value="<?php echo $k; ?>"> <?php esc_html_e( 'Import Widgets' , 'kleo_framework' ); ?></label>
										<br>
									<?php endif; ?>

									<?php if ( isset( $v['revslider'] ) ) : ?>
										<label><input type="checkbox" name="import_revslider[]" checked
										              value="<?php echo $k; ?>"> <?php esc_html_e( 'Import Revolution Slider' , 'kleo_framework' ); ?></label>
										<br>
									<?php endif; ?>

									<?php if ( isset( $v['bp_fields'] ) ) : ?>
										<label><input type="checkbox" name="import_bp_fields[]" checked
										              value="<?php echo $k; ?>"> <?php esc_html_e( 'Import Dummy Profile fields' , 'kleo_framework' ); ?></label>
										<br>
									<?php endif; ?>

									<?php if ( isset( $v['options'] ) ) : ?>
										<label><input type="checkbox" name="import_options[]" checked
										              value="<?php echo $v['options']; ?>"> <?php esc_html_e( 'Import Theme options' , 'kleo_framework' ); ?></label>
										<?php
									...
									<?php if ( isset( $v['plugins'] ) && ! empty( $v['plugins'] ) ) : ?>
										<label>
											<input type="checkbox" name="activate_plugins[]" checked value="<?php echo $k; ?>">
										</label>
										<?php
										echo esc_html__( 'Activate required plugins', 'kleo_framework' );
									...
									<?php endif; ?>
									<br>
									<small>It is recommended to leave all options checked to reproduce our demo.</small><br>
								</div>
							</div>
