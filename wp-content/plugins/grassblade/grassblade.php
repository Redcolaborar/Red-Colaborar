...
*/
define("GRASSBLADE_VERSION", "1.5.9");
...
function grassblade($attr) {
	...
	if(!empty($link_button_image)) {
	   $text = "<img src='".$link_button_image."' />";
	}
	if(empty($src))
		$return = ''; //Return blank if empty src
	else if($target == 'iframe')
	$return = "<iframe class='grassblade_iframe' frameBorder='0' src='$src' width='$width' height='$height' webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>";
	...
	$return = "<div $id class='grassblade'>".$return."</div>";
	return apply_filters("grassblade_shortcode_return", $return, $params, $shortcode_atts, $attr);
}
function grassblade_scripts() {
	global $pagenow;
	if(!is_admin() || $pagenow == "post-new.php" || $pagenow == "post.php" || in_array(@$_GET["page"], array("grassblade-lrs-settings","pageviews-settings")))
	wp_enqueue_script(
		'grassblade',
...
class grassblade_xapi_companion {
	...
	function define_fields() {
		...
		$this->fields = array(
			...
			array( 'id' => 'endpoint', 'label' => __( 'Endpoint URL', 'grassblade' ), 'placeholder' => '', 'type' => 'text', 'values'=> '', 'never_hide' => true , 'help' => __( 'provided by your LRS, check FAQ below for details', 'grassblade').". ".__("You might need to add a training slash (<b>/</b>) at the end of the url if its not already there.","grassblade")),
			...
			<?php
				foreach ($this->fields as $field) {
					if($field["type"] == "html" && @$field["subtype"] == "field_group_start") {
						echo "<tr><td colspan='2'  class='grassblade_field_group ".@$field["class"]."'>";
...
function grassblade_get_post_with_content($content_id) {
	...
	else
	{
		...
		$posts = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->posts WHERE ID IN (".implode(",", $post_ids).") AND post_status IN ('publish', 'draft') ORDER BY post_title"));
		return $posts;
	}
}
