...
function gb_leaderboard($attr) {
	...
	if(!empty($has_permissions)) {
		...
		return grassblade_leaderboard_table($leaderboard);
	}
}
function grassblade_leaderboard_table($leaderboard) {
	...
	foreach ($leaderboard as $key => $value) {
		...
		$row[__("Date", "grassblade")] = $value["timestamp"];
		$leaderboard_new[$key] = $row;
	}
	$leaderboard_new = apply_filters("gb_leaderboard_array", $leaderboard_new, $leaderboard, $ids, $shortcode_atts);
