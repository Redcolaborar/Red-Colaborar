	...
	class Swift_Performance_Lite {
		...
		public function __construct() {
			...
			if (!defined('SWIFT_PERFORMANCE_VER')){
				define('SWIFT_PERFORMANCE_VER', '2.1');
			}
			...
			add_action( 'wp_dashboard_setup', function() {
				...
				$swift_widget = array( $slug => $normal_dashboard[$slug] );
				unset( $normal_dashboard[$slug] );
		...
		public function admin_notices(){
			...
			foreach((array)$messages as $message_id => $message){
				...
				if (!isset($message['permanent']) || $message['permanent'] == false){
					...
					echo '<img src="' . SWIFT_PERFORMANCE_URI . 'images/logo.png' . '">';
				}
		...
		public static function get_menu(){
			$elements = array(
				...
				6 => array('slug' => 'critical-font', 'name' =>  __('Critical Font', 'swift-performance'), 'icon' => 'fas fa-font'),
				7 => array('slug' => 'plugin-organizer', 'name' =>  __('Plugin Organizer', 'swift-performance'), 'icon' => 'fas fa-plug'),
				8 => array('slug' => 'upgrade-pro', 'name' =>  __('Upgrade PRO', 'swift-performance'), 'icon' => 'fas fa-arrow-circle-right'),
			);
		...
		public static function header($str){
			if (!headers_sent($filename, $linenum)) {
				...
				Swift_Performance_Lite::log("Can't send header: {$str} Headers already sent in {$filename} on line {$linenum}", 6);
			}
		}
		...
		public static function upgrade_link(){
			...
			$url = ($locale == 'hu_HU' ? 'https://hu.swiftperformance.io/valts-prora/' : 'https://swiftperformance.io/upgrade-pro/');
