	...
	if ( ! empty( $course_price_type ) && $course_price_type == 'closed' ) {
		...
		if ( ! empty( $paypal_email ) ) {
			...
			if ( empty( $course_price_type ) || $course_price_type == 'paynow' ) {
				$shortcode_content = do_shortcode( '[paypal type="paynow" amount="'. $course_price .'" sandbox="'. $paypal_sandbox .'" email="'. $paypal_email .'" itemno="'. $course->ID .'" name="'. $post_title .'" noshipping="1" nonote="1" qty="1" currencycode="'. $paypal_currency .'" rm="2" notifyurl="'. $paypal_notifyurl .'" returnurl="'. $paypal_returnurl .'" imagewidth="100px" pagestyle="paypal" lc="'. $paypal_country .'" cbt="'. __( 'Complete Your Purchase', 'learndash' ) . '" custom="'. $user_id. '"]' );
				...
				$shortcode_content = do_shortcode( '[paypal type="subscribe" a3="'. $course_price .'" p3="'. $course_price_billing_p3 .'" t3="'. $course_price_billing_t3 .'" sandbox="'. $paypal_sandbox .'" email="'. $paypal_email .'" itemno="'. $course->ID .'" name="'. $post_title .'" noshipping="1" nonote="1" qty="1" currencycode="'. $paypal_currency .'" rm="2" notifyurl="'. $paypal_notifyurl .'" returnurl="'. $paypal_returnurl .'" imagewidth="100px" pagestyle="paypal" lc="'. $paypal_country .'" cbt="'. __( 'Complete Your Purchase', 'learndash' ) .'" custom="'. $user_id .'" srt="'. $srt .'"]' );
...
function learndash_adjust_date_time_display($timestamp = 0, $display_format = '' ) {
	...
	if ($timestamp != 0) {
		...
		$date_time_display = get_date_from_gmt( date('Y-m-d H:i:s', $timestamp), $display_format );
	}
