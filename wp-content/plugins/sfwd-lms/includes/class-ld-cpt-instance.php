		...
		function template_content( $content ) {
			...
			if ( ! empty( $wp->query_vars['name'] ) ) {
				...
				if ( is_course_prerequities_completed( $post->ID ) ) {
					...
					//$content = "<div id='learndash_complete_prerequisites'>". sprintf( _x( 'To take this %s, you need to complete the following %s first:%s', 'placeholders: (1) will be Course, Lesson or Quiz sigular. (2) Course sigular label, (3) link and title to prerequisites course.', 'learndash' ),  $content_type, LearnDash_Custom_Label::label_to_lower( 'course' ), '<br><a href="'.$course_link.'">'.$course_title.'</a>' ).'</div>';
					...
					SFWD_LMS::get_template( 
						...
						array(
							'current_post'			=>	$post,
							'prerequisite_post' 	=> 	get_post($course_settings['course_prerequisite']),
