...
class WDM_FCC_COMM
{
    public function __construct()
    {
        ...
        add_action('admin_head', array($this, 'wdm_course_author_table_setup'));
            ...
            add_action('woocommerce_order_status_completed', array($this, 'wdm_fcc_add_record_to_db'));
    ...
    public function course_author_menu()
    {
        ...
        add_submenu_page('learndash-lms', __('Course Author Commission', 'fcc'), __('Course Author Commission', 'fcc'), 'manage_options', 'course_author', array($this, 'course_author_page_callback'));
        //}
    }
    ...
    ?>
				<a class="nav-tab <?php echo(($current_tab == 'course_author') ? 'nav-tab-active' : '') ?> " href="?page=course_author&tab=course_author"><?php echo __('Course Author', 'fcc');
    ?></a>
		<?php 
}
        ?>
...
// To show values in Commission column
    ...
    public function wdm_fcc_add_record_to_db($order_id)
    {
        ...
        global $wpdb;
        //echo 'oreder <pre>';print_R($order);echo '</pre>';
//	$group_name = get_post_meta($order_id,'_wdm_group_name',true);
//	if($group_name == ''){
//		return;
//	}
    ...
    public function wdm_course_author_second_tab()
    {
        if (!is_super_admin()) {
            ...
            if (empty($course_authors)) {
                echo '<h3>'.__('No Course Author found', 'fcc').'</h3>';
			...
			<form method="post" action="?page=course_author&tab=commission_report">
				<table>
					<tr>
						<th><?php echo __('Select Course Author:', 'fcc');
            ?></th>
						<td>
    ...
    public function wdm_fcc_commission_report($course_author_id)
    {
        ...
        ?>
						...
						<th data-sort-initial="descending" data-class="expand">
					<?php echo __('Product / Course Name', 'fcc');
        ?>
						</th>
						<th>
    ...
    ?>
					<tr>
						<th style="float:left;"><?php echo __('Select Course Author:', 'fcc');
    ?></th>
						<td>
							<select name="wdm_course_author_id">
