<?php
$sql = "SELECT ID,post_modified FROM $table WHERE post_author = ".get_current_user_id()." AND post_type like 'sfwd-topic' AND post_status IN ('draft','publish')";
...
//			}
			...
			?></td>
			<td><?php 
				$lesson_id = get_post_meta($v->ID,'lesson_id',true);
	...
	$tags[] = $c->name;
			...
			?></td>
			<td><a href = "<?php echo add_query_arg(array("topicid" => $v->ID), $link); ?>"><img src="<?php echo plugins_url('images/edit.png',dirname(dirname( __FILE__ ))); ?>" width="25" height="25"></a></td>
