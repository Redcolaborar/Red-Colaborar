jQuery(document).ready(function($) {
  ...
  $('#mpp-activity-upload-buttons a').click(function() {
    ...
    $('select#mpp_gallery_assignment option[data-gallery_media_type="' + $(this).data('media-type') + '"][data-gallery_group_id="' + $group_id + '"]').show();
  ...
  $('body').on('gallery_list_visible', function() {
    ...
    if( $('input[name="red_group_id"]').length && !$('select#whats-new-post-in').length ) {
      $('#whats-new-form').attr('data-group_id', $('input[name="red_group_id"]').val() );
    }
    ...
    if( !$clicked_before )
      ...
      $('select#mpp_gallery_assignment option[value!="create_new"]').hide();
      $('select#mpp_gallery_assignment option[data-gallery_media_type="' + $media_type + '"]').show();
    ...
    $group_options.hide();
    $('select#mpp_gallery_assignment').val($($user_options_media[0]).attr('value'));
    if( MP.is_group == '1' ) {
      $group_options_media.show();
      $user_options.hide();

      // $options_not_media_type.hide();
      if( $group_options_media.length ) {
        $('select#mpp_gallery_assignment').val($($group_options_media[0]).attr('value'));
      } else {
    ...
    var $sel_post_in = $('select#whats-new-post-in');
    ...
    if($sel_post_in.val() == '0') {
      ...
      var $this_group_options = $('select#mpp_gallery_assignment option[data-gallery_media_type="' + $media_type + '"][data-gallery_group_id="' + $sel_post_in.val() + '"]');
    ...
    $('select#whats-new-post-in').change( function() {
      ...
      if($(this).val() == '0') {
        ...
        var $this_group_options = $('select#mpp_gallery_assignment option[data-gallery_media_type="' + $media_type + '"][data-gallery_group_id="' + $(this).val() + '"]');
