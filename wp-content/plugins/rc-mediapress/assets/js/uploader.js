...
(function( exports, $ ) {
	...
	$.extend( Uploader.prototype, {
        ...
        uploaded_media_list_for_customizer : _.template ( "<div class='mpp-uploaded-media-item red_mapp_upload_items' id='mpp-uploaded-media-item-<%= id %>' data-media-id='<%= id %>'><a href='#' class='mpp-delete-uploaded-media-item'>x</a><img src='<%= url %>' /><input type='hidden' name='mpp-media-status[<%= id %>]' value='public'><div><input placeholder='Nombre Archivo' value='<%= title %>' type='text' name='mpp-media-title[<%= id %>]'></div><div class='tags_for_mediapress_customizer'><input type='hidden' class='mpp-media-associated-tags' id='mpp-media-associated-tags<%= id %>' name='mpp-media-associated-tags[<%= id %>]' placeholder='Seleccionar etiquetas...' class='demo-default input-tags-for-media ABC' data-custom_data='efgh' value=''</div></div>" ),
        // alw custom code end started from 309
		...
		success:  function( file ) {
			// alw custom code start
                        var chk_plugin_installed = jQuery("#check_wather_customizer_enabled")[0] ;
                        ...
                        if(chk_plugin_installed)
                        {
                            var get_status_data = jQuery('.get_status_data').html() ;
                            var post_tags_for_media = jQuery('#post_tags_for_media').val() ;
                            var title = file.get('title');

                            html = this.uploaded_media_list_for_customizer({id:id, url: thumbnail.url, title: title, get_status_data:get_status_data,post_tags_for_media:post_tags_for_media });
							// alw custom code end
							main_select_arr.push(id);
		...
		complete: function() {
            ...
            // {
				...
				//jQuery('.input-tags-for-media').selectize.removeOption(1);
                //jQuery(".mpp-media-associated-tags-class").chosen();
                //data-custom_data
				//jQuery('').selectize({


					// jQuery.each( main_select_arr, function( i, val ) {
					//   jQuery('#mpp-media-associated-tags'+val).selectize({
					// 		persist: false,
					// 		createOnBlur: true,
					// 		create: true,
					// 		removeOption : true,
		...
		if( !this.media_list )
			...
			//redcolaborar
			jq('body').trigger('redcolab_upload_finish');
		...
		onAddFile: function ( file ){
			...
			//redcolaborar
			jq('body').trigger('redcolab_upload_start');
			//show loader
