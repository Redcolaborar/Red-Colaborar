...
function red_legacy_theme_post_update()
{
            ...
            foreach($mpp_media_title_arr as $key=>$value)
            {
                ...
                if(!empty($value))
                {
                    ...
                    if(count($mpp_media_associated_tags) > 0 && !empty($mpp_media_associated_tags))
                    {
											...
											error_log( "{$media_id}\n" . var_export($mpp_media_associated_tags, true) . "\n\n", 3, WP_CONTENT_DIR . '/tags.log' );
                        wp_set_post_tags( $media_id, $mpp_media_associated_tags,true );
												//
												// $inserted_tags = explode( ",", $mpp_media_associated_tags );
												// $tags = array();
												...
												// 			array_push( $tags, intval( $tid ) );
												// 		}
												// 	}
												//
												// }
												//
												// wp_set_post_terms( $media_id, $tags, 'post_tag' );
	...
	if ( false === $activity_id ) {
		...
		if( !empty( $_POST['rc_category'] ) ) {
			$rc_category = $_POST['rc_category'];
			bp_activity_add_meta( $activity_id, '_redcolab_category', $rc_category );
		}
		// if( !empty( $_POST['rc_tags'] ) ) {
		// 	$rc_tags = explode( ",", $_POST['rc_tags'] );
		// 	foreach( $rc_tags as $t ) {
		//
		// 		if( is_numeric($t) ) {
		// 			bp_activity_add_meta( $activity_id, '_redcolab_tag', $t );
		// 		} else {
		// 			$term = term_exists( $t, 'post_tag' );
		//
		// 			if ($term == 0 || $term == null) {
		...
		// 			$tid = $term['term_id'];
		// 			if( !empty($tid) && !is_wp_error($tid) ) {
		// 				bp_activity_add_meta( $activity_id, '_redcolab_tag', $tid );
		// 			}
		// 		}
		//
		// 	}
		// }
		preg_replace_callback('/[#][_0-9a-zA-Z-]+/', function ($matches) {
			$tag_clean = str_replace('#', '', $matches[0] );

			$term = term_exists( $tag_clean, 'post_tag' );
                ...
                foreach($add_video_link as $video_embed_key=>$add_video)
                {
                    ...
                    if(isset($_REQUEST['mpp-media-tags-embedded']))
                    {
                        ...
                        $mpp_media_embeded_tags_data = explode(",",$mpp_media_embeded_tag_comma_seprated);
												preg_match_all( '%[#]([_0-9a-zA-Z-])+%', $content, $tags_in_content );
