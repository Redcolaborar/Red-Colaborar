...
class BuddyBoss_Edit_Activity {
	...
	public function bp_loaded(){
		add_action( 'bp_entry_after_twitter_icon',		    array( $this, 'btn_edit_activity' ), 3, 0 );
	...
	public function print_edit_activity_template(){
      ...
      <?php do_action( 'bb_before_print_edit_activity_template' ) ?>
          ...
          <?php do_action( 'wds_redcolaborar_bp_display_the_activity_dropdown', 'main' ); ?>
	...
	public function btn_edit_activity(){
		...
		// Only display the edit button if this is not a MediaPress post: see bp_mediapress_edit.php if it is.
		if( $this->can_edit_activity() && ! $_mpp_activity_type ){
			?>

			<div class="bp-activity-menu-item normal-edit mediapress-edit-cancel" data-activity_id="<?php bp_activity_id() ;?>" style="display: none">
        ...
        <a href="#" class="button bp-secondary-action buddyboss_edit_activity_cancel" onclick="return buddypress_edit_activity_mp_cancel(this);" data-activity_id="<?php bp_activity_id() ;?>" data-target_type="activity" >
			...
			<div class="bp-activity-menu-item normal-edit">
	...
	private function can_edit_activity( $activity=false ){
		 ...
		 */
		if( current_user_can( 'level_10' ) || current_user_can( 'administrator' ) ){
			$can_edit = true;
		} else {
	...
	public function ajax_get_activity_content(){
		...
		if( $activity_content ){
			...
			$retval['category'] = bp_activity_get_meta( $activity_id, 'topic' );
		}
		$retval['error'] = '2';
		$activity = new BP_Activity_Activity( $activity_id );
		$retval['can_edit'] = $this->can_edit_activity( $activity );
		die( json_encode( $retval ) );
	}
	...
	public function ajax_save_activity_content(){
		...
		// error_log("ajax_save_activity_content :: ret2 " . var_export($retval, true) . " \n", 3, WP_CONTENT_DIR . '/uploads/bea_debug.log' );
		die( json_encode( $retval ) );
	}
	...
	private function save_activity_content( $args ){
        ...
        do_action( 'bea_before_save_activity_content', $activity->id );
				...
				// error_log("save_activity_content " . $activity->id . " : " . $args['content'] . "\n", 3, WP_CONTENT_DIR . '/uploads/bea_debug.log' );
		...
		do_action( 'bp_activity_posted_update', $args['content'], $args['user_id'], $activity->id );
