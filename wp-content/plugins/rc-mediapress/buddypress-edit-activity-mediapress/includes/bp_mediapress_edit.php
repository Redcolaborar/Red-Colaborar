...
class BP_Edit_Mediapress extends BuddyBoss_Edit_Activity {
  ...
  private $default_options = array(
	...
	public function bp_loaded(){
		add_action( 'bp_entry_after_twitter_icon',		array( $this, 'btn_edit_activity' ), 10, 0 );
  ...
  public function assets_mp() {
    ...
    wp_enqueue_script( 'alertifyjs', $assets_url . 'js/alertify.js', array(),  '1.9.0', true );
    wp_enqueue_style( 'alertify', $assets_url . 'css/alertify.css', array(),  '1.9.0');
    ...
    $settings['activity_url'] = site_url() . '/actividad';
	...
	public function option( $key ){
		...
		if ( 'editable_types' === $key ) {

			// Always include activity_update in editable types (added by WDS).
			return array_merge( $option, array( 'activity_update' ) );
		}
	...
	public function print_edit_activity_template(){
      ...
      <?php do_action( 'bb_before_print_edit_activity_template' ) ?>
        ...
        <div class="rc-edit-activity-category" style="padding-top: 20px; margin-bottom: 20px;">

          <?php do_action( 'wds_redcolaborar_bp_display_the_activity_dropdown', 'mediapress' ); ?>
	...
	public function btn_edit_activity(){
		...
		// IS this a MediaPress post?
		$_mpp_activity_type = 'media_upload' === bp_activity_get_meta( bp_get_activity_id(), '_mpp_activity_type', true );

		// Show an edit button for MediaPress posts (does not show when not MP).
		if( $this->can_edit_activity() && $_mpp_activity_type ){
			?>

      <div class="bp-activity-menu-item mediapress-edit mediapress-edit-cancel" data-activity_id="<?php bp_activity_id() ;?>" style="display: none">
				<span class="bp-activity-menu-item-icon">
					<?php wds_redcolaborar_display_svg( array(
						'title' => __( 'Cancel (Media)', 'redcolaborar' ),
						'icon'  => 'icon-cross',
						'fill'  => '#000',
					) ); ?>
				</span>

        <a href="#" class="button bp-secondary-action buddyboss_edit_activity_cancel" onclick="return buddypress_edit_activity_mp_cancel(this);" data-activity_id="<?php bp_activity_id() ;?>" data-target_type="activity" >
  				<?php esc_html_e( 'Cancelar', 'buddypress-edit-activity' ); ?>
  			</a>
			</div>

			<div class="bp-activity-menu-item mediapress-edit">
				<span class="bp-activity-menu-item-icon edit-icon" data-activity_id="<?php bp_activity_id() ;?>">
					<?php wds_redcolaborar_display_svg( array(
						'title' => __( 'Edit (Media)', 'redcolaborar' ),
						'icon'  => 'icon-gear',
						'fill'  => '#000',
					) ); ?>
				</span>

        <span class="bp-activity-menu-item-icon save-icon" data-activity_id="<?php bp_activity_id() ;?>" style="display: none">
					<?php wds_redcolaborar_display_svg( array(
						'title' => __( 'Save (Media)', 'redcolaborar' ),
						'icon'  => 'icon-floppy-disk',
						'fill'  => '#000',
					) ); ?>
				</span>

        <a href="#" class="button bp-secondary-action action-edit buddyboss_edit_activity bp-activity-menu-item-link" onclick="return buddypress_edit_activity_mp_initiate(this);" data-activity_id="<?php bp_activity_id() ;?>">
  				<?php esc_html_e( 'Editar', 'buddypress-edit-activity' ); ?>
  			</a>
			</div>
			<?php
		}
  ...
  public function ajax_save_activity_content(){
    ...
    do_action('beamp_saved_activity', $retval['content'] , get_current_user_id(), $group_id, $activity_id  );
    // if(function_exists('mpp_activity_mark_attached_media')) {
    //   mpp_activity_mark_attached_media( $activity->id );
    // }

    /*
     * This was removed by WDS as it causes duplicate Media when adding files.
     */
    ...
    // Perform activity update action.
		do_action( 'bp_activity_posted_update', $content, get_current_user_id(), $args['activity_id'] );
