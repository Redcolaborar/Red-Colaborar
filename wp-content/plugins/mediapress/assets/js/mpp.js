...
jQuery( document ).ready( function() {
    ...
    jq( document ).on( 'click', '#mpp-activity-upload-buttons a', function() {
		...
		var el = jq( this );
		//set upload context as activity  alw custom code  start
        var data_type_data = jq(el).data("media-type") ;
        if(data_type_data == 'embed_video')
        {
            makeEnterVideoLinkCallback();
            return ;
        }
        else
        {
            //jQuery('.red_video_field_wrapper').remove();
        }
       //alw custom code end
        mpp.activity_uploader.param( 'context', 'activity' );

//        var mpp_galler_data = jQuery('select[name="mpp_galler_data"] option:selected').val();
//        mpp.activity_uploader.param( 'mpp_galler_data',  mpp_galler_data);
//        if(mpp_galler_data == 'create_new')
//        {
//            var new_gallery_title = jQuery('#gallery_title_text_id').val();
//            mpp.activity_uploader.param( 'new_gallery_title',  new_gallery_title);
//        }
   ...
   //filter ajax request but only if the activity post form is present
	if( jq( '#whats-new-form' ).get(0) ) {
		...
		jq( document ).ajaxComplete( function( evt, xhr, options ) {
			 ...
			 switch( action ) {
				 ...
				 case 'post_update':
                     jQuery('.gallery_title_text').remove();     //alw custom code start
                     jQuery('#whats-new-post-in-box').hide();
                     load_more_galleries();           //alw custom code end
					 mpp.activity_uploader.hide_ui() ; //clear the list of uploaded media
					// alw custom start
					var singleText = "single=5";
					var s = window.location.search;

					if (s.indexOf(singleText) == -1) {
						//window.location.href += (s.substring(0,1) == "?") ? "&" + singleText : "?" + singleText;
					}
	...
	function is_post_update( qs ) {
		...
		if( action == 'post_update' ) {
			//alert("after55");
				//console.log("aftehr");
				mpp_mejs_activate(currentActId);
		   return true;
		}
...
function mpp_mejs_activate( activity_id ) {
	//console.log('mpp_mejs_activate11');
	...
	if( jq.fn.mediaelementplayer == undefined ) {
		...
		//console.log('mpp_mejs_activate2222');
	var settings = {};

	if ( typeof _wpmejsSettings !== 'undefined' ) {
		settings = _wpmejsSettings;
		console.log('_wpmejsSettings=== undefined');
		console.log(_wpmejsSettings);
	}

	settings.success = function (mejs) {
		var autoplay, loop;
		console.log('settings.success1');
		...
		console.log('mpp_mejs_activate333');
	};
	console.log('mpp_mejs_activate444');
	...
	console.log('mpp_mejs_activate9999');
}
 //alw custom code start
function load_more_galleries() {
        var str = 'action=getMoreGalleryData';
        jQuery.ajax({
            type: "POST",
            dataType: "html",
            url: ajaxurl,
            ...
            success: function(data){
                jQuery('#latest_gallery_data_container').html(data);
								jQuery('body').trigger('gallery_list_visible');
                var selected_gal = jQuery( "#mpp_gallery_assignment" ).val();
...
function getGoupIdByGalleryIdJS(gallery_id_data)
{
    var str = 'gallery_id='+ gallery_id_data + '&action=getGoupIdByGalleryId';
        jQuery.ajax({
            type: "POST",
            dataType: "html",
            url: ajaxurl,
            data: str,
            success: function(data){
                jQuery('#mpp_gallery_assignment').after(data);
            },
            error : function(jqXHR, textStatus, errorThrown) {
                $loader.html(jqXHR + " :: " + textStatus + " :: " + errorThrown);
            }
